...
// abandoned form still leaves a usable record. Same shape as the hero-story screen;
// `age` is here rather than inside the questionnaire because the pipeline uses it to set
// the reading level of every page.
export default function ContactScreen({ answers, setAnswers, onSubmit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!answers.name || !String(answers.name).trim()) {
      setError(isEn ? "Please enter the child's name" : 'נא למלא את שם הילד/ה');
      return;
    }
