...
// Name / age / gender + contact details, collected before the questionnaire so an
// abandoned form still leaves a usable record. Same shape as the moving-house screen;
// the extra `age` question is here rather than inside the questionnaire because the
// pipeline uses it to set the reading level of every page.
export default function ContactScreen({ answers, setAnswers, onSubmit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!answers.name || !String(answers.name).trim()) {
      setError(isEn ? "Please enter the hero's name" : 'נא למלא את שם הגיבור/ה');
      return;
    }
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center px-4 py-10 rounded-3xl" style={{ background: 'linear-gradient(135deg, #EAF8FD 0%, #FFF0F7 100%)' }}>
      ...
      >
        ...
        >
          {isEn ? 'Start the adventure →' : 'מתחילים בהרפתקה ←'}
        </button>
      </motion.form>
    </div>
  );
