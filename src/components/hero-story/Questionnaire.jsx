...
// Same flow as the moving-house questionnaire, with one fix: `showIf` there compares a
// dependency answer with `values.includes(dep)`, which only works when the answer is a
// single value. Here the dependency ("who joins the adventure") is a MULTI-select, so the
// answer is an array — includes(array) is always false and the follow-up questions
// (sibling name, pet name) would never appear. matchesShowIf handles both shapes.
...
export default function Questionnaire({ answers, setAnswers, storyId }) {
  ...
  useEffect(() => {
    trackEvent('hero_story_questionnaire_started');
  }, []);
  ...
  const goNext = () => {
    ...
    setPageError('');
    trackEvent('hero_story_questionnaire_step_completed', { step: pageIdx + 1 });
  ...
  const handleFinish = async () => {
    ...
    if (missing) { setPageError(errorFor(missing)); return; }
    setPageError('');
    trackEvent('hero_story_questionnaire_completed');
    setCreating(true);
    try { sessionStorage.setItem('storyLeap_heroStoryPending', JSON.stringify({ answers, lang })); } catch (_) {}
    try {
      const storyData = {
      ...
      // submitHeroStoryWithCredits, after the credits are actually charged. A row written
      // here would make the watcher start producing (and paying for) a book nobody bought.
      navigate(`/HeroStoryCheckout?story_id=${id}&lang=${lang}`);
    } catch (e) {
      navigate('/HeroStory');
    } finally {
      setCreating(false);
