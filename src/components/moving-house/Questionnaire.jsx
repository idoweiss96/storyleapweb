...
export default function Questionnaire({ answers, setAnswers, storyId }) {
  ...
  useEffect(() => {
    trackEvent('moving_house_questionnaire_started');
  }, []);
  ...
  const goNext = () => {
    ...
    setPageError('');
    trackEvent('moving_house_questionnaire_step_completed', { step: pageIdx + 1 });
  ...
  const handleFinish = async () => {
    ...
    if (!isPageValid()) { setPageError(pageErrorMsg); return; }
    setPageError('');
    trackEvent('moving_house_questionnaire_completed');
    setCreating(true);
    try { sessionStorage.setItem('storyLeap_movingHousePending', JSON.stringify({ answers, lang })); } catch (_) {}
    try {
      const storyData = {
      ...
      // Note: unlike KitaAlef, answers are NOT synced to a Google Sheet here — the KitaAlef
      // sheet's column layout is specific to the kindergarten questionnaire and would be
      // corrupted by moving-house answers. A dedicated sheet/export can be added later.
      navigate(`/MovingHouseStory?story_id=${id}&lang=${lang}`);
    } catch (e) {
      navigate('/MovingHouse');
    } finally {
      setCreating(false);
    }
  };
