...
// Duplicated from src/pages/KitaAlefStory.jsx (same flow/template), with moving-house
// content and its own credits function. The original page is untouched.
const PENDING_KEY = 'storyLeap_movingHousePending';

export default function MovingHouseStory() {
  ...
  const handleDetailsSubmit = async (e) => {
    ...
    if (!user) {
      const resumeUrl = storyId ? `/MovingHouseStory?story_id=${storyId}&resume=1` : '/MovingHouseStory?resume=1';
  ...
  const handleCreateStory = async () => {
    ...
    try {
      ...
      if (!id) {
        ...
        window.history.replaceState({}, '', `/MovingHouseStory?story_id=${id}`);
      } else {
      ...
      base44.analytics.track({ eventName: 'moving_house_story_submitted', properties: { story_id: id } });

      const result = await base44.functions.invoke('submitMovingHouseStoryWithCredits', { story_id: id, lang });
      if (result.data?.success) {
        ...
        base44.analytics.track({ eventName: 'moving_house_credits_used', properties: { story_id: id } });
        navigate('/MyStories');
      } else {
  ...
  const handleBuyCredits = async () => {
    ...
    try {
      ...
      base44.analytics.track({ eventName: 'moving_house_story_saved_pending_payment', properties: { story_id: id } });
      sessionStorage.removeItem(PENDING_KEY);
  ...
  if (!answers) {
    return (
      <div className="max-w-lg mx-auto py-20 text-center">
        ...
        <Button onClick={() => navigate('/MovingHouse')} className="rounded-xl bg-slate-800 hover:bg-slate-700">
          {isEn ? 'To the Moving House questionnaire' : 'לשאלון עוברים דירה'}
        </Button>
      </div>
    );
  }
    ...
    <div className="max-w-2xl mx-auto pb-12" dir={lang === 'he' ? 'rtl' : 'ltr'}>
      <div className="text-center mb-8">
        ...
        <p className="text-slate-600">
          {isEn ? 'Getting ready for the move — a personal, heartwarming story' : 'מתכוננים למעבר ביחד — סיפור אישי ומרגש'}
        </p>
      </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="p-4 rounded-xl border" style={{ background: '#EAF8FD', borderColor: '#B8EBF7' }}>
                      ...
                      <p className="text-sm text-slate-600 leading-relaxed mt-2">
                        {isEn
                          ? 'In the meantime, feel free to try our free "Our Goodbye Ritual" and "A Little Heart from Home" activities together. You\'ll find them on our homepage.'
                          : 'עד אז, אתם מוזמנים לנסות יחד את הפעילויות החינמיות שלנו: "טקס הפרידה שלנו" ו"לב קטן מהבית". אפשר למצוא אותן בדף הבית שלנו.'}
                      </p>
                    </div>
                    <Button
                ...
                <div className="flex items-center gap-4 mb-6 p-4 rounded-xl" style={{ background: 'linear-gradient(135deg, #EAF8FD, #FFF0F7)' }}>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500">
                      {isEn ? 'Getting ready to move' : 'מתכוננים למעבר'}
                    </p>
                  </div>
                </div>
