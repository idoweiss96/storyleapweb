...
// Checkout step of the prepare-story book. Same flow/template as HeroStoryCheckout.jsx,
// pointed at submitPrepareStoryWithCredits — which charges the credits and then writes the
// questionnaire row into the prepare-story order sheet the Python pipeline watches.
//
// The sheet write happens THERE and not in the questionnaire: a row written before payment
// would make the watcher start producing (and paying for) a book nobody bought.
const PENDING_KEY = 'storyLeap_prepareStoryPending';
const CREDITS_REQUIRED = 110;

export default function PrepareStoryCheckout() {
  ...
  // The subtitle under the child's name. `topic` is the display label the parent picked
  // ("🦷 ביקור אצל רופא/ת שיניים"); when they chose "Other" it is empty and their own
  // words stand in, so the screen always names the actual event rather than a placeholder.
  const topic = (Array.isArray(answers?.topic) ? answers.topic[0] : answers?.topic)
    || answers?.topic_free_text
    || '';
  ...
  const handleDetailsSubmit = async (e) => {
    ...
    if (!user) {
      const resumeUrl = storyId ? `/PrepareStoryCheckout?story_id=${storyId}&resume=1` : '/PrepareStoryCheckout?resume=1';
  ...
  const saveRecord = async () => {
    ...
    if (!id) {
      ...
      id = saved.id;
      setStoryId(id);
      window.history.replaceState({}, '', `/PrepareStoryCheckout?story_id=${id}`);
    } else {
  ...
  const handleCreateStory = async () => {
    ...
    try {
      ...
      base44.analytics.track({ eventName: 'prepare_story_submitted', properties: { story_id: id } });

      const result = await base44.functions.invoke('submitPrepareStoryWithCredits', { story_id: id, lang });
      if (result.data?.success) {
        ...
        base44.analytics.track({ eventName: 'prepare_story_credits_used', properties: { story_id: id } });
  ...
  const handleBuyCredits = async () => {
    ...
    try {
      const id = await saveRecord();
      base44.analytics.track({ eventName: 'prepare_story_saved_pending_payment', properties: { story_id: id } });
      sessionStorage.removeItem(PENDING_KEY);
  ...
  if (!answers) {
    return (
      <div className="max-w-lg mx-auto py-20 text-center">
        ...
        <Button onClick={() => navigate('/PrepareStory')} className="rounded-xl bg-slate-800 hover:bg-slate-700">
    ...
    <div className="max-w-2xl mx-auto pb-12" dir={lang === 'he' ? 'rtl' : 'ltr'}>
      <div className="text-center mb-8">
        ...
        <h1 className="text-3xl font-bold mb-2" style={{ color: '#1A1A6E' }}>
          {isEn ? `${name}'s preparation book 🌱✨` : `ספר ההכנה של ${name} 🌱✨`}
        </h1>
        <p className="text-slate-600">
          {topic || (isEn ? 'A personalized preparation book' : 'ספר הכנה מותאם אישית')}
        </p>
      </div>
            ...
            <Card className="border-0 shadow-xl" style={{ boxShadow: '0 4px 30px rgba(255,111,181,0.12)' }}>
              <CardContent className="p-6 md:p-8">
                ...
                <div className="flex items-center gap-4 mb-6 p-4 rounded-xl" style={{ background: 'linear-gradient(135deg, #EAF8FD, #FFF0F7)' }}>
                  ...
                  <div>
                    <p className="font-bold text-lg" style={{ color: '#1A1A6E' }}>{name}</p>
                    <p className="text-sm text-slate-500">{topic || (isEn ? 'Getting ready together' : 'מתכוננים ביחד')}</p>
                  </div>
                </div>
