...
// Mirrors base44/functions/submitKitaAlefStoryWithCredits/entry.ts (credit deduction +
// notification), adapted for the Moving House flow. Deliberately does NOT call
// updateKitaAlefSheetPaymentStatus — that sheet's layout is specific to the kindergarten
// questionnaire and moving-house records were never written to it.
...
async function sendStoryInProgressEmail(base44ServiceRole, email, childName, lang, gender) {
  ...
  const body = isEn
    ...
    <p style="font-size:16px;line-height:1.7;">Hi there, we've received ${childName}'s details and we're creating a special story to help them get ready for the move. We'll let you know the moment it's ready!</p>
    ...
    <p style="font-size:16px;line-height:1.7;">היי, קיבלנו את הפרטים של ${childName} ואנחנו יוצרים ${pronoun} סיפור מיוחד שילווה ${pronoun === 'עבורה' ? 'אותה' : 'אותו'} לקראת המעבר. נעדכן אתכם ברגע שהוא מוכן!</p>
